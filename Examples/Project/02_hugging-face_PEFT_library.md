...
- Convert the model into a PEFT model using the PEFT config.
...
The PEFT config specifies the adapter configuration for your parameter-efficient fine-tuning process. The base class for this is a `PeftConfig`, but this example uses a `LoraConfig`, the subclass used for low rank adaptation (LoRA).
...
Look at the LoRA adapter documentation for additional hyperparameters that can be specified by passing arguments to `LoraConfig()`. The Hugging Face LoRA conceptual guide (opens in a new tab) also contains additional explanations.
...
Then use `get_peft_model()` to get a trainable PEFT model (using the LoRA config instantiated previously):
